...
async def lifespan(app: FastAPI):
    ...
    # Bootstrap ruoli built-in (v3.4.24.1)
    try:
...
app = FastAPI(title="MediaFlow", version="3.4.24.1", lifespan=lifespan)
...
async def health():
    from app.services.ai_provider import get_provider
    p = get_provider()
    return {"status": "ok", "app": settings.app_name, "version": "3.4.24.1",
