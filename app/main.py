...
def _auto_migrate_columns():
    ...
    # v3.4.40 — QuoteLine: parent_line_id per eredità righe in versioning
...
app = FastAPI(title="MediaFlow", version="3.4.40", lifespan=lifespan)
...
async def health():
    from app.services.ai_provider import get_provider
    p = get_provider()
    return {"status": "ok", "app": settings.app_name, "version": "3.4.40",
