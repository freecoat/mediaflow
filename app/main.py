...
def _auto_migrate_columns():
    ...
    if "users" in insp.get_table_names():
        ...
        # v3.5.0-alpha.172.252 — preferenze client email (JSON)
        if "mail_prefs" not in cols:
            print("[auto-migrate] users.mail_prefs mancante -> ALTER TABLE")
            with engine.begin() as conn:
                conn.execute(text("ALTER TABLE users ADD COLUMN mail_prefs JSON NULL"))
    ...
    # v3.5.0-alpha.115 — total_cost_external (Σ supplier invoices) + accrued_stale flag
    if "job_cost_lines" in insp.get_table_names():
        jclcols = {c["name"] for c in insp.get_columns("job_cost_lines")}
        jcl_alter_115 = [
    ...
    if "delivery_items" in insp2.get_table_names():
        ...
        di_add = [
            ...
            ("requires_physical", "BOOLEAN NOT NULL DEFAULT 0"),  # α.172.205
            ("physical_media_kind", "VARCHAR(20) NULL"),  # α.172.205
        ]
        with engine.begin() as conn:
    ...
    # v3.5.0-alpha.172.182 — inspector fresco: Inspector.get_columns() è cached
    # per-tabella, riusare `insp` darebbe la lista colonne pre-ALTER (footgun).
    insp_nc = inspect(engine)
    # v3.5.0-alpha.172.182 — naming convention (tenant default + item override)
    if "tenants" in insp_nc.get_table_names():
        tcols = {c["name"] for c in insp_nc.get_columns("tenants")}
...
async def lifespan(app: FastAPI):
    ...
    # v3.5.0-alpha.172.19 — Backfill silent unit_nature su PriceItem +
    # JobDeliverable. Idempotente, safe-to-rerun. Corregge default sbagliato
    # post-migration α.172.1 (tutti deliverable_qty) e mantiene allineato il
    # mapping unit → nature in caso di update PriceItem senza UI.
    try:
        from app.database import SessionLocal
        from scripts.migrate_backfill_unit_nature import run_silent_backfill
        _db = SessionLocal()
        try:
            res = run_silent_backfill(_db)
            _db.commit()
            if res["price_items_fixed"] or res["deliverables_fixed"]:
                print(
                    f"[lifespan] unit_nature backfill: "
                    f"PriceItem={res['price_items_fixed']}, "
                    f"Deliverable={res['deliverables_fixed']}"
                )
        finally:
            _db.close()
    except Exception as e:
        print(f"[lifespan] unit_nature backfill failed: {e}")
...
app = FastAPI(title="Claqo", version="3.5.0-alpha.172.255", lifespan=lifespan)
