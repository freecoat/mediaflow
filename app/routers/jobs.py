...
@router.post("/api/reverse-extra")
async def reverse_extra(
    project_id: int = Form(...),
    mode: str = Form("new"),  # "new" | "existing"
    job_id: Optional[int] = Form(None),
    new_job_title: Optional[str] = Form(None),
    ...
    """Reverse-flow (v3.4.51): crea Job extra (o riusa) + JobCostLine(is_extra=True)
    su progetto senza quotazione. Usato dal modal booking quando il progetto
    target non ha quote.

    Modes:
      - "new": crea nuovo Job 'Extra — {project.title}' (titolo override via new_job_title)
      - "existing": aggiunge la cost line al job_id passato (deve essere reverse-flow,
        cioè quote_id=None, altrimenti errore — gli extra su job quote-driven si
        aggiungono dall'editor della quote o da /jobs/{id}/cost-lines)
    """
    from app.services.job_extras import (
        add_extra_cost_line, create_extra_job_for_project, hydrate_from_price_item,
    )
    ...
    if mode == "existing":
        if not job_id:
            raise HTTPException(400, "job_id richiesto in mode=existing")
        job = db.query(Job).filter(Job.id == job_id, Job.project_id == project_id).first()
        if not job:
            raise HTTPException(404, "Job non trovato per questo progetto")
        if job.quote_id is not None:
            raise HTTPException(
                400,
                "Il job esistente ha una quotazione collegata: aggiungi la riga "
                "dall'editor della quote, non come extra reverse."
            )
    elif mode == "new":
        job = create_extra_job_for_project(db, p, title=new_job_title)
    else:
        raise HTTPException(400, "mode deve essere 'new' o 'existing'")

    # Hydrate da price_item se valorizzato
    desc_in = (description or "").strip()
    unit_in = (unit or "day").strip() or "day"
    price_in = float(unit_price) if unit_price is not None else 0.0
    if price_item_id:
        desc_in, unit_in, price_in = hydrate_from_price_item(
            db, price_item_id, desc_in, unit_in, price_in
        )
    if not desc_in:
        raise HTTPException(400, "description o price_item_id richiesti")

    line = add_extra_cost_line(
        db, job,
        description=desc_in,
        quantity=quantity,
        unit=unit_in,
    ...
    return {
        ...
        "cost_line_description": line.description,
        "cost_line_total": line.total_expected,
        "is_new_job": mode == "new",
        "budget_quoted": job.budget_quoted,
    }
