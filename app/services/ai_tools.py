                ...
                "lines": {
                    ...
                    "items": {
                        ...
                        "properties": {
                            ...
                            "unit":          {"type": "string", "enum": ["day", "turno", "hour", "flat"]},
    ...
    },
    {
        ...
        "input_schema": {
            ...
            "properties": {
                ...
                "smart_split":      {"type": "boolean", "description": "Spezza ogni giornata in mattina+pomeriggio secondo la WorkingHoursPolicy della risorsa (es. 9-13 + 14-18 con pausa pranzo 13-14). 1 booking/giorno ma N assignments. Il CR calcola le ore al netto della pausa. PRIMA di mettere true CHIEDI all'utente se vuole pausa pranzo (anche se non l'ha citata): tipico in Italia 9-18 = pausa 13-14. Default false (back-compat)."},
            },
...
- `propose_recurring_bookings(job_id, resource_id, rule, start_date, until_date, start_time, end_time, smart_split?)` → MUTATION: crea N booking ricorrenti (lun-ven o regola custom). Le occorrenze in conflitto vengono saltate (non bloccanti). USA per "prenota Luca lun-ven 9-13 da domani al 30 maggio". Param `smart_split=true` spezza ogni giorno in mattina+pomeriggio secondo la policy della risorsa (pausa pranzo non conta nelle ore CR). Response include `overtime_warning` se la giornata effettiva eccede 8h.
...
6. **Booking ricorrenti**: per richieste tipo "online editor lun-ven 9-18 per 4 settimane", USA `propose_recurring_bookings` (un singolo Apply) invece di proporre 20 booking singoli.
   ...
   **Pausa pranzo — CHIEDI SEMPRE** (anche se l'utente non l'ha citata): prima di proporre una serie ricorrente con orari "ampi" (range > 6h o che attraversa la fascia 12:30-14:30), CHIEDI: *"orario continuato o con pausa pranzo? In Italia tipico 9-18 = pausa 13-14."*. Se l'utente conferma la pausa → passa `smart_split=true` a `propose_recurring_bookings`. Il sistema spezzerà ogni giornata in mattina+pomeriggio secondo la WorkingHoursPolicy della risorsa (es. policy default 9-13 + 14-18 produce 2 assignments da 4h, totale 8h reali per giorno). Se utente vuole continuato → passa `smart_split=false`. NON proporre senza chiarire — la differenza tra 8h e 9h sul CR è economicamente significativa.
